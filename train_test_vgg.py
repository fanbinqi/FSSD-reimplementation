...
#parser.add_argument('--resume_net', default='weights/FSSD_vgg_300/1213/FSSD_vgg_VOC_epoches_70.pth', help='resume net for retraining')
...
parser.add_argument('-we', '--warm_epoch', default=5,
                    type=int, help='max epoch for retraining')
        ...
        if (iteration % epoch_size == 0):
            ...
            if epoch % args.save_frequency == 0 and epoch > 0:
                torch.save(net.state_dict(), os.path.join(save_folder, args.version + '_' + args.dataset + '_epoches_' +
                                                          repr(epoch) + '.pth'))
            if epoch % args.test_frequency == 0 and epoch >= 70:
                net.eval()
...
def adjust_learning_rate(optimizer, gamma, epoch, step_index, iteration, epoch_size):
    ...
    if epoch <= args.warm_epoch:
        ...
        lr = 1e-6 + (args.lr - 1e-6) * 0.5 * (1.0 + math.cos((iteration-args.warm_epoch*epoch_size) * math.pi/((args.max_epoch-args.warm_epoch)*epoch_size)))
